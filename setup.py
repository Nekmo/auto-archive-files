...
ARCHIVER_TEMPLATE_FILENAME = 'archiver-template.json'
ARCHIVER_FILE = '/etc/auto-archive-files/{}'.format(ARCHIVER_TEMPLATE_FILENAME)

SERVICES = [('services/archiver.service', '/usr/lib/systemd/system/archiver.service'),
            ('services/archiver-template.timer', '/usr/lib/systemd/system/archiver-template.timer')]
...
PACKAGE_NAME = 'auto-archive-files'
PACKAGE_DOWNLOAD_URL = 'https://github.com/Nekmo/auto-archive-files/archive/master.zip'  # .tar.gz
URL = 'https://github.com/Nekmo/auto-archive-files'  # Project url
STATUS_LEVEL = 1  # 1:Planning 2:Pre-Alpha 3:Alpha 4:Beta 5:Production/Stable 6:Mature 7:Inactive
KEYWORDS = ['linux', 'unix', 'monitor', 'alert', 'auto-archive-files', 'move', 'files', 'archiver']
...
class SystemInstallCommand(install):
    ...
    def run(self):
        ...
        if getpass.getuser() != 'root':
            print('WARNING: Auto-Archive-Files installed as "{}". Install as root to create the system files!')
            return
        ...
        print('Copying archiver template file')
        os.makedirs(os.path.dirname(ARCHIVER_FILE), exist_ok=True)
        if not os.path.lexists(ARCHIVER_FILE):
            shutil.copy(os.path.join(dir_path, ARCHIVER_TEMPLATE_FILENAME), ARCHIVER_FILE)
